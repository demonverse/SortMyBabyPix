"""
Consolidates photos from multiple drives into one destination, organized as
Photos/YYYY/, using EXIF capture date where available (falling back to the
file's modified date), and skipping exact duplicate files.
...
def gather_sources(destination):
    ...
    print("Second drive/folder to scan:")
    sources.append(ask_path("  Path"))

    while ask_yes_no("\nAny other drives you want scanned?"):
        sources.append(ask_path("  Path"))
