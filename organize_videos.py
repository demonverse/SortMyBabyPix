...
REPORT_FILENAME = "videos_report.txt"
...
EPOCH_MS_PATTERN = re.compile(r"\b(1[5-9]\d{11})\b")
...
def append_report(video_root, text):
    with open(video_root / REPORT_FILENAME, "a", encoding="utf-8") as f:
        f.write(text + "\n")
...
def main():
    ...
    append_report(video_root, f"=== Run started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===")
    ...
    for i, source in enumerate(sources, 1):
        ...
        print(progress)
        append_report(video_root, progress)
    ...
    print(final)
    append_report(video_root, final)

    print(f"Organized videos are in: {video_root}")
    print(f"Report saved to:         {video_root / REPORT_FILENAME}")
